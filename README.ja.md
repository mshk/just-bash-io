...
| --- | ---: | ---: | ---: | ---: |
| Markdown | 27,883 → 3,714 | 109.52秒 → 4.20秒 | 比較不可 | 比較不可 |
| CSV | 18,712 → 2,406 | 71.89秒 → 2.68秒 | 87.1% | 96.4% |
| HTML | 33,573 → 2,500 | 138.81秒 → 2.59秒 | 92.6% | 98.0% |

Markdown は通常方式の3回すべてで転記ミスが発生し、just-bash-io は3回すべて完全一致しました（完全一致件数: 0/3 → 3/3）。
Markdown 行は通常方式の失敗した出力も含む実測値です。出力が異なるため、削減率は比較不可としています。
合成データの文字列置換に限定した測定です。
